...
char enqueue_message(uint8_t m)
{
    ...
    if (!isQueueFull())
    {
		...
		mydata->message[mydata->tail].data[COLORR] = mydata->color;
...
int getFirstDiffPosition(int a, int b){
	int z = a^b;
	int i=0;
	while((1&z) == 0){
		z>>=1;
	...
	int pos_value = getPosVal(b,i);
	int color = i<<1;
	color += pos_value;
	return color;
}

void setting_color(uint8_t i)
{
	if(i == 0)
		set_color(RGB(1,0,0));
	if(i == 5)
...
void coloring(uint8_t *payload)
{	
	if(payload[ID]==mydata->my_left)
		mydata->color = getFirstDiffPosition(payload[COLORR],mydata->color); //call luqman's function
	
	if(mydata->color < 6)
	{
		//set color
		#ifdef SIMULATOR
			printf("ID = %d & Final Color = %d \n", mydata->my_id, mydata->color);
		#endif
		
		setting_color(mydata->color);
	}
	else
	{
		mydata->set_color = 1;
...
void selectFirstAvailable(uint8_t left_color, uint8_t right_color)
{
	if(left_color!=0 && right_color!=0){
		mydata->color = 0;
		#ifdef SIMULATOR
			printf("#################My ID->%d  MY Color -> %d  \n",mydata->my_id,mydata->color);
		#endif
	}
	else if(left_color!=1 && right_color!=1){
		mydata->color = 1;
		#ifdef SIMULATOR
			printf("###############My ID->%d  MY Color -> %d  \n",mydata->my_id,mydata->color);
		#endif
	}
	else if(left_color!=2 && right_color!=2){
		mydata->color = 2;
		#ifdef SIMULATOR
			printf("###################My ID->%d  MY Color -> %d  \n",mydata->my_id,mydata->color);
		#endif
...
void recv_sharing_color(uint8_t *payload)
{
	
	if(mydata->color > 5 && mydata->state == COOPERATIVE && mydata->my_left == payload[ID])
	{

		
		coloring(payload);
	}
	else if(mydata->color == payload[COLORR])
	{
		
		mydata->color=mydata->my_id;
	}	
	
	if(mydata->color < 6 && mydata->my_left == payload[ID])
	{
		mydata->my_left_color = payload[COLORR];
	}
	
	if(mydata->color < 6 && mydata->my_right == payload[ID])
	{
		mydata->my_right_color = payload[COLORR];
	}
	
	if(mydata->color < 6 && mydata->color > 2)
	{
		selectFirstAvailable(mydata->my_left_color, mydata->my_right_color);
	}
    ...
    if (m->type == NORMAL && m->data[MSG] !=NULL_MSG)
    {
        ...
        switch (m->data[MSG])
        {
			...
			case ELECTED:
				...
				break;
			case COLOR:
				//if(mydata->my_right != m->data[LEADER])
					coloring(m->data);
					break;
	    //case ELECTED:
		//elected(m->data);
			case SHARE:
				if(mydata->my_id != m->data[ID])
					recv_sharing_color(m->data);
				break;
        }
    }
}
...
void send_joining()
{
...

    {
		...
		/*if(mydata->my_id > 5 && mydata->state == COOPERATIVE )
		{
			#ifdef SIMULATOR
				printf("Starting Coloring\n");
			#endif
			mydata->color = mydata->my_id;
			enqueue_message(COLOR);
			mydata->set_color = 1;
		}*/
    }
}
...
void start_color()
{
	if(mydata->set_color && !isQueueFull())
	{
		enqueue_message(COLOR);
		mydata->set_color = 0;
	}
