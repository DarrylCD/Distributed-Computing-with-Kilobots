...
void recv_joining(uint8_t *payload)
{
    ...
    if (payload[RIGHT] == mydata->my_id) // && payload[LEFT] == my_left)
    {
        ...
        mydata->red = 1;
            ...
            mydata->sendToken = mydata->now + TOKEN_TIME;
    }
...
{
    // Precondition
    if (mydata->now >= mydata->nextShareSending &&  mydata->message_sent == 1)
    {
        // Sending
...
void send_move()
{
    ...
    if(mydata->token && mydata->state == COOPERATIVE && mydata->sendToken <= mydata->now && mydata->message_sent ==1)
    {
...
void setup() {
    ...
    mydata->green = 0,
    mydata->blue = 0,
    mydata->send_moving = 0;
    mydata->sendToken = mydata->now;

    mydata->msg.data[MSG] = NULL_MSG;
    mydata->msg.crc = message_crc(&mydata->msg);
    
    //mydata->send_moving = rand_soft() < 64  ? 1 : 0;
    //mydata->blue = mydata->send_moving;
    if(rand_soft() < 80)
        mydata->token = 1;
    if(mydata->token){
        mydata->blue = 1;
